genie3MPI = function(data,nodes,pathv,regulatorIndex=NULL,hosts=NULL){
  #initialize MPI
  library('Rmpi');
  ...
  foldslave <- function() {
    ...
    while (tag != 2) {
      ...
      if(is.null(regulatorIndex)){
        ...
        res <- genie3(y=data[,foldNumber],x=data[,-foldNumber]);
        ...
        if(foldNumber%in%regulatorIndex){
          ...
          res <- NA;
          set.seed(foldNumber)
          try(res <- genie3(y=data[,foldNumber],x=data[,regulatorIndex][,-wi]),silent=TRUE)
          if(!is.na(res)){
            temp_vbsr[-wi] <- res;
            #temp_cor[-wi] <- res$cor;
          }
          ...
          res <- NA;
          set.seed(foldNumber)
          try(res <- genie3(data[,foldNumber],data[,regulatorIndex]),silent=TRUE);
          if(!is.na(res)){
            temp_vbsr <- res;
            #temp_cor <- res$cor;
          }
  ...
  network <- network[,-1]
  save(network,file=paste(pathv,'result_genie3.rda',sep=''));
