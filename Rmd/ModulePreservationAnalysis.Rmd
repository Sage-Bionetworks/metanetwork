...
source('../R/lib/makeModulePreservationSubmissions.R')
...
```{r overlap.stats, fig.height = 10, fig.width=20, eval=F}
...
```{r preservation.stats, fig.height=10, fig.width=20, eval=T}
# Write input files to sge as RData
for (ref in names(networks))
  for (test in names(networks))
    if (ref != test){
      # Package data to send to sge queue
      makeModulePreservationSubmissions(ref = ref,
                                        test = test,
                                        refNet = networks[[ref]], 
                                        testNet = networks[[test]], 
                                        refModLabels = modules[[ref]], 
                                        testModLabels = modules[[test]], 
                                        refExp = exp[[ref]], 
                                        testExp = exp[[test]],
                                        n=100)
      }    
...
### Consolidate preservation statistics results
```{r preservation.stats.results, fig.height=10, fig.width=20, eval=F}
...
```{r plot.data, fig.height=6, fig.width=20, eval=F}
load('temp.RData')
